...
interface CommitHistoryResult {
  ...
  totalCommits: number;
  isAbandoned: boolean;
  abandonmentThresholdDays: number;
...
class GitHubMCPServer {
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      try {
        ...
        if (name === 'cloneResurrectionTarget') {
          const result = await this.cloneResurrectionTarget(args.url as string);
          return {
            content: [
              {
                type: 'text',
        ...
        if (name === 'getCommitHistory') {
          const result = await this.getCommitHistory(
            args.repoPath as string,
            (args.abandonmentThresholdDays as number) || 365
          );
          return {
            content: [
              {
   ...
   */
  private async cloneResurrectionTarget(url: string): Promise<CloneResult> {
    try {
      // Validate URL
   ...
   */
  private async getCommitHistory(
    repoPath: string,
    abandonmentThresholdDays: number = 365
   ...
   */
  private prepareAuthenticatedUrl(url: string): string {
    if (!this.githubToken) {
      return url;
    }
